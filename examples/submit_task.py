...
def parse_args():
    parser = argparse.ArgumentParser(description="Submit a task to the Task Queue System")
    parser.add_argument("--name", type=str, default="example_task", help="Name of the task")
    ...
    parser.add_argument("--payload", type=str, default='{"action": "print", "message": "Hello, World!"}',
...
def submit_task(args):
    ...
    try:
        ...
        print(f"Error submitting task: {e.response.status_code} {e.response.reason_phrase}")
...
def check_task_status(api_url, task_id):
    try:
        ...
        print(f"Error checking task status: {e.response.status_code} {e.response.reason_phrase}")
        print(f"Response: {e.response.text}")
        sys.exit(1)
