...
async def test_pause_and_resume_task(db_session):
    ...
    # Pause the task
    paused_task = await TaskQueueService.pause_task(db=db_session, task_id=created_task.id)
    assert paused_task.status == TaskStatus.PAUSED
    
    # Resume the task
    resumed_task = await TaskQueueService.resume_task(db=db_session, task_id=created_task.id)
